	...
	<div id="__next" data-reactroot="">
		<div class="mt-20 flex justify-center px-6 pb-16 sm:mt-32 sm:px-0">
			<div class="w-full max-w-md">
				<!-- 사진이 안뜸 이상함 -->
				<h1 class="mt-6 text-center text-2xl font-bold sm:text-3xl">아이디
					찾기</h1>


				<p class="mt-2 text-center text-sm text-gray-600 dark:text-gray-400"
					style="margin-top: 20px;">
					회원가입시 입력하신 이메일 주소를 입력하시면,<br> 해당 이메일로 인증번호를 보내드립니다.
				</p>
				<div class="mx-4 sm:mx-0">
					...
					<form class="mt-8 space-y-6" action="returnId.do" method="POST" onsubmit="checkId(event);">
						<div>
								...
								<div>
									...
									<div class="mt-1">
										...
										<div
											...
											<button type="button" class="btn" disabled="disabled">발송</button>
										</div>
									</div>

									<div class="mt-1 jm" style="display: none;">
										<input id="emailChk" type="text" required
											onchange="valiFormNum(this)"
											class="block w-full appearance-none rounded-md border border-gray-500/30 px-3 py-2 text-sm placeholder-gray-500/80 shadow-sm focus:border-gray-500 focus:outline-none focus:ring-0 dark:bg-gray-500/20 sm:text-base"
						...
						<div
							...
							<button type="submit" id="searchIdSub" disabled="disabled"
								class="flex w-full justify-center rounded-md border border-transparent bg-blue-500 px-4 py-2 text-sm font-medium text-white hover:bg-blue-400 focus:outline-none disabled:bg-blue-500 disabled:opacity-40">아이디
								찾기</button>
						</div>
		...
		$('.chkBtn').click(function() {
			var confirm = $('#emailChk').val();
		 	$.ajax({
				...
				type : 'post',
				data : {
					mEmailChk : confirm, //내가쓴 인증번호 값
					mAuthkey : checkNum //내가쓴 인증번호 값
				},
				dataType : 'json',
				success : function(result) {
					if (result == 1) {
						alert("인증번호 일치");
						$('#searchIdSub').attr("disabled",false);
					} else {
