...
public class MemberDAO {
	...
	// 아이디 중복확인
	private final String SQL_CHECK_ID = "SELECT * FROM MEMBER WHERE ID=?;";
	// 닉네임 중복확인
	private final String SQL_CHECK_NICKNAME = "SELECT * FROM MEMBER WHERE MNICKNAME=?;";
	// 아이디찾기
	...
	private final String SQL_SELECTALL_REPORT = 
			"SELECT M.*, A.GRADE, A.REPORTCNT \n"
					...
					+ "ON M.ID=A.BID;";
	// 정보변경
	private final String SQL_UPDATE_INFO = "UPDATE MEMBER2 SET PW=?, MNAME=?, MNICKNAME=?, MEMAIL=? WHERE ID=?;";
	// 회원정지
